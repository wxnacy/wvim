...
iabbrev <buffer> classm public class{}
                 ...
                 \<esc>kk$Fsli
iabbrev <buffer> sout System.out.println();<esc>hi""<esc>i
iabbrev <buffer> soutf System.out.printf();<esc>hi""<esc>i
iabbrev <buffer> if if () {}<esc>i<cr><esc>kF)i
iabbrev <buffer> fori for (int i = 0; i <; i++) {}<esc>i<cr><esc>k$F;i
iabbrev <buffer> foreach for (:) {}<esc>i<cr><esc>k$F:i
...
function! NewJavaFile()
    ...
    let l:name = expand("%:r")
    call setline(1, "/**")
    call append(1, " * Author: " . g:wvim_author)
    call append(2, " */")
